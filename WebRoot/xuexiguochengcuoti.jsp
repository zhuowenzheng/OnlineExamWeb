...
if(a.equals("insert")){
    // 执行插入学习过程错题模块,表名：xuexiguochengcuoti
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
    String charuid = new CommDAO().insert(request,"xuexiguochengcuoti",ext);  // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    if(!Request.get("referer").equals("")){ // 提交了跳转地址，则跳转到这个地址中
     ...
     // 执行更新学习过程错题模块,表名：xuexiguochengcuoti
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
    String charuid = new CommDAO().update(request,"xuexiguochengcuoti",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("xuexiguochengcuoti").find(Request.get("id"));
    new CommDAO().delete(request,"xuexiguochengcuoti");  // 执行删除命令
