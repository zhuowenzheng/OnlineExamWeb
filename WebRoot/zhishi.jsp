...
if(a.equals("insert")){
    ...
    ext.put("zhishineirong", util.DownloadRemoteImage.run(Request.get("zhishineirong")));// 将前台编辑器内容的图片远程下载下来并保存在本地中
    ext.put("tianjiashijian", Info.getDateStr());
    String charuid = new CommDAO().insert(request,"zhishi",ext);  // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
    ext.put("zhishineirong", util.DownloadRemoteImage.run(Request.get("zhishineirong")));// 将前台编辑器内容的图片远程下载下来并保存在本地中
    String charuid = new CommDAO().update(request,"zhishi",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("zhishi").find(Request.get("id"));
    new CommDAO().delete(request,"zhishi");  // 执行删除命令
